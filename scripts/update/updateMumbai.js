const PROXIES = {
	EtherOrcsPoly: "0x9Ee5F6C8B02908a29f111cA9B7B93e61d2374ab1",
...
task("update", "Update a contract implementation")
	...
	.setAction(async (args, hre) => {
		...
		let contract = PROXIES[args.contract];
		...
		if (hre.network.name === "localhost") {
			...
			await network.provider.send("hardhat_setStorageAt", [
				PROXIES[contract],
				"0x1",
				hre.ethers.utils.hexZeroPad(newOwner.address, 32),
			]);
		}

		console.log("Deploying", contract);
		const implFact = await hre.ethers.getContractFactory(contract);
		const impl = await implFact.deploy();
		await impl.deployed();
		console.log("Implementation: ", impl.address);

		console.log("Updating proxy");
		const proxy = await hre.ethers.getContractAt("Proxy", PROXIES[contract]);

		const tx = await proxy.setImplementation(impl.address);
		await tx.wait();
