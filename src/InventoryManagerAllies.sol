...
contract InventoryManagerAllies {
	...
	function getTokenURI(
		...
		if (class_ == 1) {
			...
			return
				_buildShamanURI(
					_getUpper(id_),
					getSVG(body_, featA_, featB_, helm_, mainhand_, offhand_),
					getAttributes(details_, level_, modF_, skillCredits_)
				);
		}
	}

	function _buildShamanURI(
		bytes memory upper,
		string memory svg,
	...
	function _shaman(bytes22 details)
		internal
		pure
		returns (
			uint8 body_,
	...
	{
		address source = part == Part.body ? bodies[id] : part == Part.featA
			? featA[id]
			: part == Part.featB
			? featB[id]
			: part == Part.helm
		...
		returns (bytes memory data)
	{
		string memory s = string(
			abi.encodePacked(
				part == Part.body ? "body" : part == Part.featA
					? "featA"
					: part == Part.featB
					? "featB"
					: part == Part.helm
	...
	function getAttributes(
		...
		return
			string(
				abi.encodePacked(
					_getTopAtt(details_),
					_getBottomAtt(level_, sc_, modF_)
				)
			);
	}
		...
		return
			string(
				abi.encodePacked(
					...
					toString(level_),
					'},{"trait_type": "skillCredits", "value":',
					toString(sc_),
					'},{"display_type": "boost_number","trait_type": "Herbalism", "value":',
					toString(modF_),
					"}]"
				)
			);
	...
	function getBodyAttributes(uint256 body_)
		internal
		pure
	...
	function getFeatAAttributes(uint256 featA_)
		internal
		pure
		returns (string memory)
	{
	...
	function getFeatBAttributes(uint256 featB_)
		internal
		pure
		returns (string memory)
	{
		return
			string(
				abi.encodePacked(
					...
					getFacialHairName(featB_),
					'"}'
				)
			);
	}

	function getHelmAttributes(uint256 helm_)
		internal
		pure
		returns (string memory)
	{
		return
			string(
				abi.encodePacked(
					...
					toString(getTier(uint8(helm_))),
					"}"
				)
			);
	}

	function getMainhandAttributes(uint256 mainhand_)
		internal
		pure
		returns (string memory)
	{
		return
			string(
				abi.encodePacked(
					...
					toString(getTier(uint8(mainhand_))),
					"}"
				)
			);
	}

	function getOffhandAttributes(uint256 offhand_)
		internal
		pure
		returns (string memory)
	{
		return
			string(
				abi.encodePacked(
					...
					toString(getTier(uint8(offhand_))),
					"}"
				)
			);
	}
