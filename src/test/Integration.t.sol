...
contract TestRaids is OrcsBaseTest {
    ...
    function test_sendToRaidStaked() public {
        ...
        orcsPoly.sendToRaid(myIds, raidLocaction, false, new uint256[](myIds.length), new uint256[](myIds.length));

        for (uint256 index = 0; index < myIds.length - 1; index++) {
            emit log_named_uint("id", myIds[index + 1]);
            assertEq(raidsPoly.commanders(index + 1), address(this));
            (,, uint256 end,, ) = logCampaing(index + 1);
            emit log_named_uint("times", block.timestamp + 192 hours);
            assertEq(end, block.timestamp + 192 hours);
        }
        ...
        for (uint256 index = 0; index < myAllyIds.length - 1; index++) {
            assertEq(raidsPoly.commanders(index + 11051), address(this));
            (,, uint256 end, ,) = logCampaing(index + 11051);
            assertEq(end, 1636580705 + (192 hours * 2));
        }
    }
