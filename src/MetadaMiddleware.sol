...
contract MetadataMiddleware {
	...
	function getTokenURI(
		...
		(, bytes memory ret) = address(this).staticcall(
			abi.encodeWithSignature(
				"getSVG(uint8,uint8,uint8,uint8)",
				body_,
				helm_,
				mainhand_,
				offhand_
	...
	function getBodyAttributes(uint8 body_)
		internal
		view
		returns (string memory)
	{
		NameHandlerLike handler = NameHandlerLike(
			body_ > 52 ? equipmentHandler : impl
		);
		return
			string(
				abi.encodePacked(
					'{"trait_type":"Body","value":"',
					handler.getBodyName(body_),
					'"}'
				)
			);
	}

	function getHelmAttributes(uint8 helm_)
		internal
		view
		returns (string memory)
	{
		NameHandlerLike handler = NameHandlerLike(
			helm_ > 52 ? equipmentHandler : impl
		);
		return
			string(
				abi.encodePacked(
					'{"trait_type":"Helm","value":"',
					handler.getHelmName(helm_),
	...
	function getMainhandAttributes(uint8 mainhand_)
		internal
		view
		returns (string memory)
	{
		NameHandlerLike handler = NameHandlerLike(
			mainhand_ > 52 ? equipmentHandler : impl
		);
		return
			string(
				abi.encodePacked(
					'{"trait_type":"Mainhand","value":"',
					handler.getMainhandName(mainhand_),
	...
	function getOffhandAttributes(uint8 offhand_)
		internal
		view
		returns (string memory)
	{
		NameHandlerLike handler = NameHandlerLike(
			offhand_ > 52 ? equipmentHandler : impl
		);
		return
			string(
				abi.encodePacked(
					'{"trait_type":"Offhand","value":"',
					handler.getOffhandName(offhand_),
	...
	function getChampionsAttribute(uint256 id)
		internal
		view
		returns (string memory)
	{
		uint256 daysAsChampion = NameHandlerLike(nameHandler).timeAsChampion(id) /
			1 days;
		return
			string(
				abi.encodePacked(
					'{"display_type":"number","trait_type":"DaysAsTopChampion","value":"',
					toString(daysAsChampion),
					'"}'
				)
			);
	}
	...
	function _getName(uint256 orcId) internal view returns (string memory) {
		if (nameHandler == address(0))
			return string(abi.encodePacked("Orc #", toString(orcId)));
