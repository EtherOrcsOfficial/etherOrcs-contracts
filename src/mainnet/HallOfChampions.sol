...
contract HallOfChampions {
	...
	function getName(uint256 orcId) public view returns (string memory) {
		...
		if (joined[orcId] == 0)
			return string(abi.encodePacked("Orc #", _toString(orcId)));

		// If Orc has only a fisrt name
		if (
			bytes(_firstNames[orcId]).length > 0 &&
			bytes(_lastNames[orcId]).length == 0
	...
	function getChampions()
		...
		returns (
			uint256[] memory ids,
			uint256[] memory timeSinceJoined,
			string[] memory names,
			uint256[] memory tributePaid
		)
	{
		ids = champions;
	...
	function timeAsChampion(uint256 id)
		external
		view
		returns (uint256 timeInSeconds)
	{
		return
			timeAtTop[id] + (isTopFive[id] ? block.timestamp - topEnterTime[id] : 0);
	}
	...
	function overrideName(
		uint256 orcId,
		string calldata newFirstName,
		string calldata newLastName
	) external {
		require(msg.sender == admin);
	...
	function changeName(
		...
		require(
			msg.sender == owner || msg.sender == etherOrcs.ownerOf(orcId),
			"Not the orc owner"
		);
		...
		require(
			firstLength == 0 || validateName(firstName_),
			"Not a valid first name"
		);
		require(
			lastLength == 0 || validateName(lastName_),
