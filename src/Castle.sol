...
contract Castle {
	...
	function travel(
		...
		bytes[] memory calls = new bytes[](
			(orcsLen > 0 ? orcsLen + 1 : 0) +
				(allieslen > 0 ? allieslen + 1 : 0) +
				(zugAmount > 0 ? 1 : 0) +
				(shrAmount > 0 ? 1 : 0)
		);
		...
		if (orcsLen > 0) {
			...
			calls[orcsLen] = abi.encodeWithSelector(
				this.unstakeMany.selector,
				reflection[orcs],
				msg.sender,
				orcIds
			);
		...
		if (allieslen > 0) {
			...
			calls[currIndex + allieslen] = abi.encodeWithSelector(
				this.unstakeMany.selector,
				reflection[allies],
				msg.sender,
				alliesIds
		...
		if (zugAmount > 0) {
			...
			calls[currIndex] = abi.encodeWithSelector(
				this.mintToken.selector,
				reflection[address(zug)],
				msg.sender,
				zugAmount
			);
			currIndex++;
	...
	function _buildData(uint256 id) internal view returns (bytes memory data) {
		(
			...
			uint16 zM,
			uint32 lP
		) = OrcishLike(orcs).orcs(id);
		data = abi.encodeWithSelector(
			this.callOrcs.selector,
			abi.encodeWithSelector(
				OrcishLike.manuallyAdjustOrc.selector,
				id,
				b,
				h,
				m,
	...
	function _buildDataAllies(uint256 id)
		internal
		view
		returns (bytes memory data)
	{
		(
			...
			uint8 sc,
			bytes22 d
		) = OrcishLike(allies).allies(id);
		data = abi.encodeWithSelector(
			this.callAllies.selector,
			abi.encodeWithSelector(
				OrcishLike.adjustAlly.selector,
				id,
				cl,
				l,
				lP,
	...
	function _stake(
		...
		require(
			(token == orcs ? orcOwner[id] : allyOwner[id]) == address(0),
			"already staked"
		);
		require(msg.sender == token, "not orcs contract");
		require(
			ERC721Like(token).ownerOf(id) == address(this),
			"orc not transferred"
		);
