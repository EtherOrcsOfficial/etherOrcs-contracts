...
contract RaidsPoly {
	...
	struct Raid {
		...
		uint16 regReward;
		uint16 grtReward;
		uint16 supReward; // Rewards are scale down to 100(= 1BS & 1=0.01) to fit uint16.
	}
	...
	function _getBaseOutcome(
		...
		prob =
			minProb +
			((((orcLevel - minLevel) * HND_PCT) / (maxLevel - minLevel)) *
				(maxProb - minProb)) /
			HND_PCT;
	}

	function _getLevelBonus(uint256 maxLevel, uint256 orcLevel)
		internal
		pure
		returns (uint256 prob)
	{
		if (orcLevel <= maxLevel) return 0;
		if (orcLevel <= maxLevel + 20)
			return ((((orcLevel - maxLevel) * HND_PCT) / 20) * 500) / HND_PCT;
		prob = 500;
	}

	function _getChampionBonus(uint16 orcId)
		internal
		view
