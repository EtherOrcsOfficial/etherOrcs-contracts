...
contract RaidsPoly {
	...
	function init(
		...
		Raid memory crookedCrabBeach = Raid({
			minLevel: 5,
			maxLevel: 5,
			duration: 48,
			cost: 60,
		...
		Raid memory twistedPirateCove = Raid({
			minLevel: 15,
			maxLevel: 25,
			duration: 30,
			cost: 45,
		...
		Raid memory warpedSpiderDen = Raid({
			minLevel: 25,
			maxLevel: 35,
			duration: 72,
			cost: 90,
		...
		Raid memory toxicQuagmire = Raid({
			minLevel: 45,
			maxLevel: 45,
			duration: 96,
			cost: 170,
		...
		Raid memory evilMerfolkCastle = Raid({
			minLevel: 50,
			maxLevel: 75,
			duration: 144,
			cost: 225,
	...
	function unstake(uint256 orcishId) public {
		...
		require(
			msg.sender == (orcishId < 5051 ? address(orcs) : address(allies)),
			"Not orcs contract"
		);
	...
	function _claim(uint256 id) internal {
		...
		if (cmp.reward > 0 && _ended(campaigns[id])) {
			...
			if (cmp.seed != 0) {
				...
				reward =
					_getReward(raid, id, level, rdn, "RAID") +
					(cmp.double ? _getReward(raid, id, level, rdn, "DOUBLE RAID") : 0);
			}
	...
	function _stake(uint256 id, address owner) internal {
		...
		require(
			(id < 5051 ? orcs.ownerOf(id) : allies.ownerOf(id)) == address(this),
			"orc not transferred"
		);
	...
	function _startCampaign(
		...
		require(
			potions_ <= (double ? raid.maxPotions * 2 : raid.maxPotions),
			"too much potions"
		);
		require(
			potions_ >= (double ? raid.minPotions * 2 : raid.minPotions),
			"too much potions"
		);
		require(
			msg.sender == (orcishId < 5051 ? address(orcs) : address(allies)),
	...
	function _ended(Campaign memory cmp) internal view returns (bool) {
		return
			...
			(
				giantCrabHealth == 0
					? cmp.end - (cmp.double ? 2 days : 1 days)
					: cmp.end
			);
	}
	...
	function _getReward(
		...
		uint256 ramdom,
		string memory salt
	) internal view returns (uint176 reward) {
		uint256 rdn = (uint256(keccak256(abi.encode(ramdom, orcId, salt))) %
			10_000) + 1;

		uint256 champBonus = _getChampionBonus(uint16(orcId));

		uint256 greatProb = _getBaseOutcome(
			raid.minLevel,
			raid.maxLevel,
			raid.grtAtMin,
			raid.grtAtMax,
			...
			_getLevelBonus(raid.maxLevel, orcLevel) +
			champBonus;
		uint256 superbProb = _getBaseOutcome(
			raid.minLevel,
			raid.maxLevel,
			raid.supAtMin,
			raid.supAtMax,
		...
		reward =
			uint176(
				rdn <= superbProb ? raid.supReward : rdn <= greatProb + superbProb
					? raid.grtReward
					: raid.regReward
			) *
			1e16;
	...
	function _getBaseOutcome(
		...
		prob =
			minProb +
			((((orcishLevel - minLevel) * HND_PCT) / (maxLevel - minLevel)) *
				(maxProb - minProb)) /
			HND_PCT;
	}

	function _getLevelBonus(uint256 maxLevel, uint256 orcishLevel)
		internal
		pure
		returns (uint256 prob)
	{
		if (orcishLevel <= maxLevel) return 0;
		if (orcishLevel <= maxLevel + 20)
			return ((((orcishLevel - maxLevel) * HND_PCT) / 20) * 500) / HND_PCT;
		prob = 500;
	}
