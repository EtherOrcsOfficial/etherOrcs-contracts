...
contract EtherOrcsPoly is PolyERC721 {
	...
	function setImplementer(bytes4[] calldata funcs, address source)
		external
		onlyOwner
	{
	...
	function doActionWithManyOrcs(uint256[] calldata ids, Actions action_)
		external
	{
	...
	function _claim(uint256 id) internal noCheaters {
		...
		if (action.action == Actions.FARMING)
			zug.mint(action.owner, claimableZug(timeDiff, orc.zugModifier));
	...
	function pillage(
		...
		bool tryMainhand,
		bool tryOffhand
	) public isOwnerOfOrc(id) noCheaters {
		require(
			block.timestamp >= uint256(activities[id].timestamp),
			"on cooldown"
		);
		...
		}
		{
			...
			if (tryMainhand) {
				(pool, item) = _getItemFromPool(
					pool,
					_randomize(rand_, "MAINHAND", id)
				);
		...
		// Update zug modifier
		Orc memory orc = orcs[id];
		uint16 zugModifier_ = _tier(orc.helm) +
			_tier(orc.mainhand) +
			_tier(orc.offhand);
	...
	function sendToRaid(
		...
		for (uint256 index = 0; index < ids.length; index++) {
			if (activities[ids[index]].action != Actions.UNSTAKED)
				_doAction(ids[index], msg.sender, Actions.UNSTAKED, msg.sender);
	...
	function returnFromRaid(uint256[] calldata ids, Actions action_)
		external
		noCheaters
	{
	...
	function pull(address owner_, uint256[] calldata ids) external {
		...
		for (uint256 index = 0; index < ids.length; index++) {
			if (activities[ids[index]].action != Actions.UNSTAKED)
				_doAction(ids[index], owner_, Actions.UNSTAKED, owner_);
	...
	function addLootPools() external {
		...
		LootPool memory dungeon = LootPool({
			minLevel: 3,
			minLootTier: 2,
			cost: 0,
			total: 1000,
		...
		LootPool memory crypt = LootPool({
			minLevel: 10,
			minLootTier: 4,
			cost: 120,
			total: 1000,
	...
	function claimable(uint256 id) external view returns (uint256 amount) {
		uint256 timeDiff = block.timestamp > activities[id].timestamp
			...
			: 0;
		amount = activities[id].action == Actions.FARMING
			? claimableZug(timeDiff, orcs[id].zugModifier)
	...
	function _getItemFromPool(LootPool memory pool, uint256 rand)
		internal
		pure
		returns (LootPool memory, uint8 item)
	{
		uint256 draw = rand % pool.total--;
