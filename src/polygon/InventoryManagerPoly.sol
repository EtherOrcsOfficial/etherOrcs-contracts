...
contract InventoryManagerPoly {
	...
	function getSVG(
		...
		// it's a unique!
		if (helm_ > 40)
			return string(abi.encodePacked(header, get(Part.unique, body_), footer));
	...
	{
		address source = part == Part.body ? bodies[id] : part == Part.helm
			? helms[id]
			: part == Part.mainhand
			? mainhands[id]
			: part == Part.offhand
		...
		returns (bytes memory data)
	{
		string memory s = string(
			abi.encodePacked(
				part == Part.body ? "body" : part == Part.helm
					? "helm"
					: part == Part.mainhand
					? "mainhand"
					: part == Part.offhand
	...
	function getOffhandName(uint8 id) public pure returns (string memory) {
		...
		if (id < 20) {
			if (id < 10) {
				...
				if (id < 7)
					return id == 5 ? "Wooden Shield +1" : "Paper Hands Shield +1";
				return
					id == 7 ? "Dagger +1" : id == 8 ? "Pirate Hook +1" : "Offhand Axe +2";
			}
			if (id <= 15) {
				if (id < 13) {
					return
