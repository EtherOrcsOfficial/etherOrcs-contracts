...
contract EtherOrcsAlliesPoly is PolyERC721 {
	...
	modifier ownerOfAlly(uint256 id, address who_) {
		require(
			ownerOf[id] == who_ || activities[id].owner == who_,
			"not your ally"
		);
		_;
	}
	...
	function initialize(
		...
		Location memory swampHealerHut = Location({
			minLevel: 25,
			skillCost: 5,
			cost: 0,
			tier_1Prob: 88,
		...
		Location memory enchantedGrove = Location({
			minLevel: 31,
			skillCost: 5,
			cost: 0,
			tier_1Prob: 50,
		...
		Location memory jungleHealerHut = Location({
			minLevel: 35,
			skillCost: 25,
			cost: 0,
			tier_1Prob: 85,
		...
		Location memory monkTemple = Location({
			minLevel: 35,
			skillCost: 20,
			cost: 0,
			tier_1Prob: 80,
		...
		Location memory forgottenDesert = Location({
			minLevel: 40,
			skillCost: 35,
			cost: 0,
			tier_1Prob: 85,
		...
		Location memory moldyCitadel = Location({
			minLevel: 45,
			skillCost: 30,
			cost: 0,
			tier_1Prob: 75,
		...
		Location memory swampEnchanterDen = Location({
			minLevel: 55,
			skillCost: 45,
			cost: 200,
			tier_1Prob: 40,
		...
		Location memory theFallsOfTruth = Location({
			minLevel: 55,
			skillCost: 45,
			cost: 200,
			tier_1Prob: 70,
		...
		Location memory ethereanPlains = Location({
			minLevel: 60,
			skillCost: 50,
			cost: 200,
			tier_1Prob: 80,
		...
		Location memory djinnOasis = Location({
			minLevel: 60,
			skillCost: 10,
			cost: 150,
			tier_1Prob: 70,
		...
		Location memory spiritWorld = Location({
			minLevel: 70,
			skillCost: 60,
			cost: 300,
			tier_1Prob: 30,
	...
	function claimable(uint256 id) external view returns (uint256 amount) {
		uint256 timeDiff = block.timestamp > activities[id].timestamp
			...
			: 0;
		amount = activities[id].action == 1
			? _claimable(timeDiff, allies[id].modF)
	...
	function doAction(uint256 id, uint8 action_)
		public
		ownerOfAlly(id, msg.sender)
		noCheaters
	{
	...
	function _doAction(
		...
		uint88 timestamp = uint88(
			block.timestamp > action.timestamp ? block.timestamp : action.timestamp
		);
	...
	function doActionWithManyAllies(uint256[] calldata ids, uint8 action_)
		external
	{
	...
	function startJourneyWithManyAllies(
	...
	function endJourneyWithManyAllies(
		uint256[] calldata ids,
		uint8 place,
		uint8 equipment
	) external {
	...
	function _claim(uint256 id) internal noCheaters {
		...
		if (action.action == 1)
			potions.mint(action.owner, POTION_ID, _claimable(timeDiff, ally.modF));
	...
	function startJourney(
		...
		uint8 place,
		uint8 equipment
	) public isOwnerOfAlly(id) noCheaters {
		...
		journeys[id] = Journey({
			seed: OracleLike(gamingOracle).request(),
			location: place,
			equipment: equipment
		});
	}
	...
	function sendToRaid(
		...
		for (uint256 index = 0; index < ids.length; index++) {
			if (activities[ids[index]].action != 0)
				_doAction(ids[index], msg.sender, 0, msg.sender);
			_transfer(msg.sender, raids, ids[index]);
		}
		RaidsLikePoly(raids).stakeManyAndStartCampaign(
			ids,
			msg.sender,
			location_,
			double_,
	...
	function startRaidCampaign(
		...
		for (uint256 index = 0; index < ids.length; index++) {
			require(
				msg.sender == RaidsLikePoly(raids).commanders(ids[index]) &&
					ownerOf[ids[index]] == address(raids),
				"not staked or not your orc"
			);
		}
		RaidsLikePoly(raids).startCampaignWithMany(
			ids,
			location_,
			double_,
			potions_
	...
	function returnFromRaid(uint256[] calldata ids, uint8 action_)
		external
		noCheaters
	{
		require(action_ < 3, "invalid action");
		...
		for (uint256 index = 0; index < ids.length; index++) {
			require(
				msg.sender == raidsContract.commanders(ids[index]),
				"not your orc"
			);
	...
	function pull(address owner_, uint256[] calldata ids) external {
		...
		for (uint256 index = 0; index < ids.length; index++) {
			if (activities[ids[index]].action != 0)
				_doAction(ids[index], owner_, 0, owner_);
	...
	function _equipShaman(
		Shaman memory sh,
		Location memory loc,
		uint256 id,
		uint256 equipment,
		uint256 rdn
	) internal pure returns (bytes22 details) {
	...
	function _getItem(Location memory loc, uint256 rand)
		internal
		pure
		returns (uint8 item)
	{
		...
		uint8 tier = uint8(
			draw <= loc.tier_3Prob
				? loc.tier_3
				: draw <= loc.tier_2Prob + loc.tier_3Prob
				? loc.tier_2
				: loc.tier_1
		);
	...
	function _claimable(uint256 timeDiff, uint256 herbalism_)
		internal
		pure
		returns (uint256 potionAmount)
	{
		potionAmount =
			(timeDiff * (0.5 ether + (herbalism_ * 0.05 ether))) /
			1 days;
	}
